...
def _shorten_name_of_logrecord(record: logging.LogRecord) -> logging.LogRecord:
    ...
    if record.name.startswith("electrum_hms."):
...
def _delete_old_logs(path, *, num_files_keep: int):
    files = sorted(list(pathlib.Path(path).glob("electrum_hms_log_*.log")), reverse=True)
...
def _configure_file_logging(log_directory: pathlib.Path, *, num_files_keep: int):
    ...
    PID = os.getpid()
    _logfile_path = log_directory / f"electrum_hms_log_{timestamp}_{PID}.log"
...
#       as possible.
# Note: some users might use Electrum-HMS as a python library and not use run_electrum_hms,
...
electrum_logger = logging.getLogger("electrum_hms")
...
def get_logger(name: str) -> logging.Logger:
    if name.startswith("electrum_hms."):
        name = name[9:]
...
def configure_logging(config: 'SimpleConfig', *, log_to_file: Optional[bool] = None) -> None:
    ...
    _logger.info(f"Electrum-HMS version: {ELECTRUM_VERSION} - https://hemis.tech - {GIT_REPO_URL}")
