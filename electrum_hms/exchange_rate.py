...
class BTXPro(ExchangeBase):
    async def get_rates(self, ccy):
        json1 = await self.get_json('api.btxpro.com', '/v1.1/public/getticker?market=btc-hms')
        if ccy != "BTC":
...
class Coinbase(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.coinbase.com',
                             '/v2/exchange-rates?currency=HMS')
        return {ccy: to_decimal(rate) for (ccy, rate) in json["data"]["rates"].items()}

class CoinCap(ExchangeBase):

    async def get_rates(self, ccy):
        json = await self.get_json('api.coincap.io', '/v2/assets/hemis/')
    ...
    async def request_history(self, ccy):
        ...
        history = await self.get_json('api.coincap.io',
                                      '/v2/assets/hemis/history?interval=d1&limit=2000')
        return dict([(timestamp_to_datetime(h['time']/1000, utc=True).strftime('%Y-%m-%d'), str(h['priceUsd']))
                     for h in history['data']])

class CoinEx(ExchangeBase):
    async def get_rates(self, ccy):
        json1 = await self.get_json('api.coinex.com', '/v1/market/ticker?market=hmsbtc')
        if ccy != "BTC":
            json2 = await self.get_json('api.coingecko.com', '/api/v3/simple/price?ids=bitcoin&vs_currencies=%s' % ccy)
...
class CoinGecko(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.coingecko.com',
                                   '/api/v3/simple/price?ids=hemis&vs_currencies=%s' % ccy)
        return {ccy: to_decimal(json['hemis'][ccy.lower()])}
    ...
    async def request_history(self, ccy):
        ...
        history = await self.get_json('api.coingecko.com',
                                      f"/api/v3/coins/hemis/market_chart?vs_currency={ccy}&days={num_days}")
...
class CryptoCompare(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ...
        result = await self.get_json('min-api.cryptocompare.com', '/data/price?fsym=HMS&tsyms={}&extraParams=ElectrumHMS'.format(tsyms))
    ...
    async def request_history(self, ccy):
        result = await self.get_json('min-api.cryptocompare.com', '/data/histoday?fsym=HMS&tsym={}&limit=100&aggregate=1&extraParams=ElectrumHMS'.format(ccy))
...
class Upbit(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json1 = await self.get_json('api.upbit.com', '/v1/ticker?markets=BTC-HMS')
        if ccy != "BTC":
            json2 = await self.get_json('api.coingecko.com', '/api/v3/simple/price?ids=bitcoin&vs_currencies=%s' % ccy)
