...
class SimpleConfig(Logger):
    ...
    def electrum_path(self):
        ...
        self.logger.info(f"electrum-hms directory {path}")
        return path
...
Warning: setting this to too low will result in lots of payment failures."""),
    ...
    GUI_QT_HISTORY_TAB_SHOW_TOOLBAR = ConfigVar('show_toolbar_history', default=True, type_=bool)
    GUI_QT_ADDRESSES_TAB_SHOW_TOOLBAR = ConfigVar('show_toolbar_addresses', default=True, type_=bool)
    ...
    BLOCK_EXPLORER = ConfigVar(
        'block_explorer', default='system default', type_=str,
...
def read_user_config(path: Optional[str]) -> Dict[str, Any]:
    """Parse and store the user config settings in electrum-hms.conf into user_config[]."""
    if not path:
        return {}
