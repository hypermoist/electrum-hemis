...
try:
    ...
    from electrum_hms._vendor import pyperclip

import electrum_hms
from electrum_hms.gui import BaseElectrumGui
from electrum_hms.bip21 import parse_bip21_URI
from electrum_hms.util import format_satoshis, format_time
from electrum_hms.util import EventListener, event_listener
from electrum_hms.bitcoin import is_address, address_to_script, COIN
from electrum_hms.transaction import PartialTxOutput
from electrum_hms.wallet import Wallet, Abstract_Wallet
from electrum_hms.wallet_db import WalletDB
from electrum_hms.storage import WalletStorage
from electrum_hms.network import NetworkParameters, TxBroadcastError, BestEffortRequestFailed
from electrum_hms.interface import ServerAddr
from electrum_hms.invoices import Invoice
...
class ElectrumGui(BaseElectrumGui, EventListener):
    ...
    def __init__(self, *, config: 'SimpleConfig', daemon: 'Daemon', plugins: 'Plugins'):
        ...
        if not storage.file_exists():
            print("Wallet not found. try 'electrum-hms create'")
            exit()
    ...
    def network_dialog(self):
        ...
        if out:
            ...
            if out.get('server') or out.get('proxy') or out.get('proxy user') or out.get('proxy pass'):
                new_proxy_config = electrum_hms.network.deserialize_proxy(out.get('proxy')) if out.get('proxy') else proxy_config
                if new_proxy_config:
