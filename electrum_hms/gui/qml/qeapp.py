...
import electrum_hms
from electrum_hms import version, constants
from electrum_hms.i18n import _
from electrum_hms.logging import Logger, get_logger
from electrum_hms.bip21 import BITCOIN_BIP21_URI_SCHEME, LIGHTNING_URI_SCHEME
from electrum_hms.base_crash_reporter import BaseCrashReporter, EarlyExceptionsQueue
    ...
    def doNotify(self, wallet_name, message):
        try:
            ...
            icon = (os.path.dirname(os.path.realpath(__file__))
                    + '/../icons/electrum.png')
            notification.notify('Electrum-HMS', message, app_icon=icon, app_name='Electrum-HMS')
        except ImportError:
...
class ElectrumQmlApplication(QGuiApplication):
    ...
    def __init__(self, args, *, config: 'SimpleConfig', daemon: 'Daemon', plugins: 'Plugins'):
        ...
        not_loaded = QFontDatabase.addApplicationFont('electrum_hms/gui/qml/fonts/PTMono-Regular.ttf') < 0
        not_loaded = QFontDatabase.addApplicationFont('electrum_hms/gui/qml/fonts/PTMono-Bold.ttf') < 0 and not_loaded
        if not_loaded:
        ...
        self.context.setContextProperty('UI_UNIT_NAME', {
            "FEERATE_SAT_PER_VBYTE": electrum_hms.util.UI_UNIT_NAME_FEERATE_SAT_PER_VBYTE,
            "FEERATE_SAT_PER_VB":    electrum_hms.util.UI_UNIT_NAME_FEERATE_SAT_PER_VB,
            "TXSIZE_VBYTES":         electrum_hms.util.UI_UNIT_NAME_TXSIZE_VBYTES,
            "MEMPOOL_MB":            electrum_hms.util.UI_UNIT_NAME_MEMPOOL_MB,
        })
