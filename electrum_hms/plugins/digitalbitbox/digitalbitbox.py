# ----------------------------------------------------------------------------------
# Electrum-HMS plugin for the Digital Bitbox hardware wallet by Shift Devices AG
# digitalbitbox.com
#
...
from electrum_hms.util import to_string, UserCancelled, UserFacingException, bfh
from electrum_hms.network import Network
from electrum_hms.logging import get_logger
from electrum_hms.plugin import runs_in_hwd_thread, run_in_hwd_thread
...
class DigitalBitbox_Client(HardwareClientBase):
    ...
    def check_device_dialog(self):
        ...
        if not self.setupRunning:
            self.isInitialized = True # Wallet exists. Electrum-HMS code later checks if the device matches the wallet
    ...
    def dbb_generate_wallet(self):
        ...
        filename = ("Electrum-HMS-" + time.strftime("%Y-%m-%d-%H-%M-%S") + ".pdf")
