...
try:
    ...
    from electrum_hemis._vendor import pyperclip

import electrum_hemis
from electrum_hemis.gui import BaseElectrumGui
from electrum_hemis.bip21 import parse_bip21_URI
from electrum_hemis.util import format_satoshis, format_time
from electrum_hemis.util import EventListener, event_listener
from electrum_hemis.bitcoin import is_address, address_to_script, COIN
from electrum_hemis.transaction import PartialTxOutput
from electrum_hemis.wallet import Wallet, Abstract_Wallet
from electrum_hemis.wallet_db import WalletDB
from electrum_hemis.storage import WalletStorage
from electrum_hemis.network import NetworkParameters, TxBroadcastError, BestEffortRequestFailed
from electrum_hemis.interface import ServerAddr
from electrum_hemis.invoices import Invoice
    ...
    def network_dialog(self):
        ...
        if out:
            ...
            if out.get('server') or out.get('proxy') or out.get('proxy user') or out.get('proxy pass'):
                new_proxy_config = electrum_hemis.network.deserialize_proxy(out.get('proxy')) if out.get('proxy') else proxy_config
                if new_proxy_config:
