...
BITCOIN_BIP21_URI_SCHEME = 'hemis'
...
def parse_bip21_URI(uri: str) -> dict:
    ...
    if ':' not in uri:
        if not bitcoin.is_address(uri):
            raise InvalidBitcoinURI("Not a hemis address")
        return {'address': uri}

    u = urllib.parse.urlparse(uri)
    if u.scheme.lower() != BITCOIN_BIP21_URI_SCHEME:
        raise InvalidBitcoinURI("Not a hemis URI")
    address = u.path
    ...
    if address:
        if not bitcoin.is_address(address):
            raise InvalidBitcoinURI(f"Invalid hemis address: {address}")
        out['address'] = address
    if 'amount' in out:
        am = out['amount']
        try:
            ...
            if amount > TOTAL_COIN_SUPPLY_LIMIT_IN_BTC * COIN:
                raise InvalidBitcoinURI(f"amount is out-of-bounds: {amount!r} HMS")
