...
docker build \
    $DOCKER_BUILD_FLAGS \
    -t electrum-hms-android-builder-img \
...
if [ ! -z "$ELECBUILD_COMMIT" ] ; then
    ...
    FRESH_CLONE=${FRESH_CLONE:-"/tmp/electrum_build/android/fresh_clone/electrum-hms"}
...
docker run -it --rm \
    --name electrum-hms-android-builder-cont \
    -v "$PROJECT_ROOT_OR_FRESHCLONE_ROOT":/home/user/wspace/electrum-hms \
